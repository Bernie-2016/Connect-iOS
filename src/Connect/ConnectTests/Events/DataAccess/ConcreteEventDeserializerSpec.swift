...
class ConcreteEventDeserializerSpec : QuickSpec {
    override func spec() {
        describe("ConcreteEventDeserializer") {
            ...
            beforeEach {
                subject = ConcreteEventDeserializer()
            }
            ...
            it("deserializes the events correctly") {
                ...
                var events = subject.deserializeEvents(jsonDictionary)
            ...
            context("when there's not enough hits") {
                it("should not explode") {
                    var events = subject.deserializeEvents([String: AnyObject]())
                    expect(events.count).to(equal(0))

                    events = subject.deserializeEvents(["hits": [String: AnyObject]()])
                    expect(events.count).to(equal(0))

                    events = subject.deserializeEvents(["hits": [ "hits": [String: AnyObject]()]])
                    expect(events.count).to(equal(0));
                }
            }
        }
