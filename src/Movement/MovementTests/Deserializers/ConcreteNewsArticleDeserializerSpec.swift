...
class ConcreteNewsArticleDeserializerSpec : QuickSpec {
    ...
    override func spec() {
        describe("ConcreteNewsArticleDeserializer") {
            beforeEach {
                self.subject = ConcreteNewsArticleDeserializer(stringContentSanitizer: FakeStringContentSanitizer())
            }
            ...
            it("deserializes the news items correctly with the sanitizer") {
                ...
                expect(newsArticleA.title).to(equal("On the Road for Bernie in Iowa SANITIZED!"))
                ...
                expect(newsArticleA.excerpt).to(equal("Larry Cohen excerpt SANITIZED!"))
                ...
                expect(newsArticleB.title).to(equal("Labor Day 2015: Stand Together and Fight Back SANITIZED!"))
                expectedDate = dateFormatter.dateFromString("07-09-2015")
                expect(newsArticleB.date).to(equal(expectedDate))
                expect(newsArticleB.body).to(equal("Labor Day is a time for honoring the working people of this country. SANITIZED!"))
                expect(newsArticleB.excerpt).to(equal("Labor Day excerpt SANITIZED!"))
            ...
            context("when title, body or url are missing") {
                it("should not explode and ignore stories that lack them") {
                    ...
                    let newsArticleA = newsArticles[0]
                    expect(newsArticleA.title).to(equal("This is good news SANITIZED!"))
                }
            }
